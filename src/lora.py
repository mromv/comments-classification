...
def make_training_pipeline(
    ...
    config = instantiate(lora_config)
...
def main(cfg: DictConfig):
    ...
    output_dir = os.path.join(cfg["general"]["output_dir"], cfg["encoder"]["short_name"], "lora")
    checkpoints_dir = os.path.join(cfg["general"]["checkpoints"], cfg["encoder"]["short_name"], "lora")
    ...
    # logging
    log.info(f"PEFT result: {cfg['encoder']['short_name']} with loss {cfg['loss']['_target_'].split('.')[-1]}")
    log.info(f"{trainer.model.print_trainable_parameters()}")
